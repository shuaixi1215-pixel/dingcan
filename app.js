const dishes = [
  { id: "bowl-beef", category: "热销", name: "招牌牛肉饭", price: 28, tag: "人气", icon: "饭", desc: "慢炖牛肉、溏心蛋、青菜，酱香浓郁。" },
  { id: "noodle-chicken", category: "热销", name: "鸡汤鲜面", price: 24, tag: "暖胃", icon: "面", desc: "清鸡汤底，搭配手撕鸡和时蔬。" },
  { id: "dumpling", category: "小吃", name: "煎饺拼盘", price: 18, tag: "酥脆", icon: "饺", desc: "猪肉白菜和三鲜双口味，外皮焦香。" },
  { id: "tofu", category: "热菜", name: "家常豆腐", price: 22, tag: "下饭", icon: "豆", desc: "豆腐煎香后入味，微辣可选。" },
  { id: "shrimp", category: "热菜", name: "鲜虾滑蛋", price: 32, tag: "新鲜", icon: "虾", desc: "鲜虾仁配嫩滑鸡蛋，适合全家分享。" },
  { id: "salad", category: "轻食", name: "牛油果鸡胸沙拉", price: 26, tag: "低脂", icon: "沙", desc: "鸡胸肉、牛油果、玉米和油醋汁。" },
  { id: "tea", category: "饮品", name: "柠檬冷萃茶", price: 12, tag: "解腻", icon: "茶", desc: "清爽茶香，搭配鲜切柠檬。" },
  { id: "set", category: "套餐", name: "双人安心套餐", price: 68, tag: "省心", icon: "套", desc: "两份主食、一份小吃、两杯饮品。" }
];

const state = {
  category: "全部",
  cart: JSON.parse(localStorage.getItem("dingcan_cart") || "{}"),
  orders: JSON.parse(localStorage.getItem("dingcan_orders") || "[]")
};
...
function save() {
  localStorage.setItem("dingcan_cart", JSON.stringify(state.cart));
  localStorage.setItem("dingcan_orders", JSON.stringify(state.orders));
}

function getCartLines() {
  return Object.entries(state.cart)
    .map(([id, qty]) => ({ ...dishes.find((dish) => dish.id === id), qty }))
...
function renderMenu() {
  const visible = state.category === "全部"
    ? dishes
    : dishes.filter((dish) => dish.category === state.category);
...
orderForm.addEventListener("submit", (event) => {
  ...
  const order = {
    ...
    items: totals.lines.map(({ name, qty, price }) => ({ name, qty, price })),
  ...
  state.orders = [order, ...state.orders].slice(0, 8);
  state.cart = {};
  orderForm.reset();
  save();
  renderCart();
